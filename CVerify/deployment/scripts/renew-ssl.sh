...
echo "[renew-ssl] Attempting certificate renewal for cverify.com.vn (and api.cverify.com.vn if it's a SAN on the same cert)..."
sudo certbot renew --quiet --deploy-hook "systemctl reload nginx"

echo "[renew-ssl] Current certificate status:"
sudo certbot certificates | grep -A 5 "cverify.com.vn" || echo "  (no matching certificate found — has certbot --nginx -d cverify.com.vn -d www.cverify.com.vn -d api.cverify.com.vn been run yet?)"
