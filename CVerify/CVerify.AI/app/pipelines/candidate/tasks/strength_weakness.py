...
class StrengthWeaknessAnalyzer(BaseTask):
    @property
    def name(self) -> str:
        return "L2-003"
    ...
    def task_name(self) -> str:
        return "StrengthWeaknessAnalyzer"
    ...
    def input_keys(self) -> List[str]:
        return ["skillProficiencies", "unmatchedCvSkills"]

    @property
    def output_keys(self) -> List[str]:
        return ["strongestDomains", "skillGaps", "overallStrengthSummary"]
    ...
    async def _execute_internal(self, context: PipelineContext, correlation_id: str) -> Dict[str, Any]:
        ...
        user = self.prompt_factory.get_strength_weakness_prompt(legacy_inputs)
        raw, telemetry = await self.claude_service.analyze_repo_with_telemetry(system, user, correlation_id)
        
        data = self._extract_json(raw)
        return {
            "strongestDomains": data.get("strongestDomains", []),
            "skillGaps": data.get("skillGaps", []),
            "overallStrengthSummary": data.get("overallStrengthSummary", "")
        }
