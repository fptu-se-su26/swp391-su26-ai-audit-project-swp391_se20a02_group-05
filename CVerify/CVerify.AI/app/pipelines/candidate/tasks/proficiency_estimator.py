...
class SkillProficiencyEstimator(BaseTask):
    @property
    def name(self) -> str:
        return "L2-002"
    ...
    def task_name(self) -> str:
        return "SkillProficiencyEstimator"
    ...
    def input_keys(self) -> List[str]:
        return ["mappedSkills", "skillEvidenceGraph", "cv"]
    ...
    async def _execute_internal(self, context: PipelineContext, correlation_id: str) -> Dict[str, Any]:
        ...
        user = self.prompt_factory.get_skill_proficiency_estimator_prompt(legacy_inputs)
        raw, telemetry = await self.claude_service.analyze_repo_with_telemetry(system, user, correlation_id)
        
        data = self._extract_json(raw)
        return {
            "skillProficiencies": data.get("skillProficiencies", [])
        }
