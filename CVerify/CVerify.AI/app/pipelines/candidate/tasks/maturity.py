...
class EngineeringMaturityAssessor(BaseTask):
    @property
    def name(self) -> str:
        return "L2-007"
    ...
    def task_name(self) -> str:
        return "EngineeringMaturityAssessor"

    @property
    def input_keys(self) -> List[str]:
        return ["repoIntelligenceReport", "maturityInputs"]

    @property
    def output_keys(self) -> List[str]:
        return ["engineeringMaturityScore", "maturityLevel", "maturitySummary", "maturitySignals"]
    ...
    async def _execute_internal(self, context: PipelineContext, correlation_id: str) -> Dict[str, Any]:
        ...
        user = self.prompt_factory.get_engineering_maturity_prompt(legacy_inputs)
        raw, telemetry = await self.claude_service.analyze_repo_with_telemetry(system, user, correlation_id)
        
        data = self._extract_json(raw)
        
        score_val = data.get("engineeringMaturityScore")
        try:
        ...
        return {
            "engineeringMaturityScore": score,
            "maturityLevel": data.get("maturityLevel", "Practitioner"),
            "maturitySummary": data.get("maturitySummary", ""),
            "maturitySignals": data.get("signals", [])
        }
