  ...
  return (
    <div className="space-y-2 mt-2 px-1 select-none">
      <div className="flex justify-between items-center text-xs">
        <span className="text-zinc-500 dark:text-zinc-400 text-[11px] font-medium font-sans">Password Strength</span>
        <span className={`font-bold text-[11px] transition-colors ${textColor}`}>{label}</span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-[11px] text-zinc-500 dark:text-zinc-400 mt-1.5">
        <span className={`flex items-center gap-1 transition-colors ${checks.length ? 'text-success font-medium' : 'text-zinc-400 dark:text-zinc-500'}`}>
          {checks.length ? "✓" : "○"} At least 8 characters
        </span>
        <span className={`flex items-center gap-1 transition-colors ${checks.uppercase ? 'text-success font-medium' : 'text-zinc-400 dark:text-zinc-500'}`}>
          {checks.uppercase ? "✓" : "○"} One uppercase letter
        </span>
        <span className={`flex items-center gap-1 transition-colors ${checks.lowercase ? 'text-success font-medium' : 'text-zinc-400 dark:text-zinc-500'}`}>
          {checks.lowercase ? "✓" : "○"} One lowercase letter
        </span>
        <span className={`flex items-center gap-1 transition-colors ${checks.digit ? 'text-success font-medium' : 'text-zinc-400 dark:text-zinc-500'}`}>
          {checks.digit ? "✓" : "○"} One number
        </span>
        <span className={`flex items-center gap-1 transition-colors ${checks.special ? 'text-success font-medium' : 'text-zinc-400 dark:text-zinc-500'}`}>
...
function ContinueWithEmailContent() {
  ...
  const handleResendOtp = async () => {
    ...
    if (result.success && result.data) {
      ...
      toast.danger(result.error?.message || "Failed to resend OTP. Please try again.");
    }
  };
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    if (!otpCode || otpCode.length < 6) {
      toast.danger("Invalid Code", { description: "Please enter the full 6-digit OTP code." });
      return;
    }

    setIsOtpLoading(true);
    const result = await verifyOtp(challengeId, email, otpCode, 'Authentication');
    setIsOtpLoading(false);

    if (result.success && result.data) {
      setVerificationToken(result.data.verificationToken);
      setStep(2);
      toast.success("OTP Verified", { description: "Email ownership proven. Please set your password." });
    } else {
      toast.danger("Verification Failed", {
        description: result.error?.message || "The OTP code entered is incorrect or expired."
      });
    }
  };
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      toast.success("Identity Setup Complete", {
        description: "Welcome to CVerify! You have been logged in successfully."
      });
      router.push(callbackUrl);
    } else {
      let errorMsg = result.error?.message || "Failed to establish password credential.";
    ...
    <Card className="w-full">
      {step === 1 ? (
        <div className="w-full flex flex-col items-center">
          ...
          <Form className="w-full flex flex-col gap-6 items-center px-12" onSubmit={handleVerifyOtp}>
            <div className="flex flex-col gap-2 items-center w-full">
              <Label className="text-muted">One-Time Code</Label>
              <InputOTP maxLength={6} value={otpCode} onChange={setOtpCode}>
                <InputOTP.Group>
                  <InputOTP.Slot className="border rounded-2xl h-12 w-12" index={0} />
                  <InputOTP.Slot className="border rounded-2xl h-12 w-12" index={1} />
                  <InputOTP.Slot className="border rounded-2xl h-12 w-12" index={2} />
                </InputOTP.Group>
                <InputOTP.Separator />
                <InputOTP.Group>
                  <InputOTP.Slot className="border rounded-2xl h-12 w-12" index={3} />
                  <InputOTP.Slot className="border rounded-2xl h-12 w-12" index={4} />
                  <InputOTP.Slot className="border rounded-2xl h-12 w-12" index={5} />
                </InputOTP.Group>
              </InputOTP>
            </div>
        ...
        <div className="w-full flex flex-col items-center">
          ...
          <Form className="w-full flex flex-col gap-6 p-12" onSubmit={handlePasswordSubmit}>
            <TextField
              isRequired
              name="password"
              type="password"
            ...
            <Button
              ...
              isDisabled={!isPasswordValid || password !== confirmPassword || isPasswordLoading}
              className="h-12 rounded-2xl"
            >
              {({ isPending }) => (
                <>
                  {isPending && <Spinner color="current" size="sm" />}
                  {isPending ? "Complete Registration..." : "Complete Registration"}
                </>
              )}
            </Button>
          </Form>
