...
import { NavigationGroupItem, NavigationNode } from '@/types/navigation.types';
...
export const SidebarGroup: React.FC<SidebarGroupProps> = ({ group, collapsed, isMobile, depth = 0 }) => {
  ...
  const expandedKeys = new Set(
    Object.keys(expandedGroups).filter((key) => expandedGroups[key] && key === group.id)
  );
              ...
              <Accordion.Trigger className={[
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <span className={[
                    ...
                    hasActiveDescendant ? "text-foreground font-bold" : "text-muted"
                  ].join(' ')}>
                    {label}
                  </span>
                </div>
            ...
            {group.children.map((child) => {
              if (child.type === 'item') {
                return (
                  ...
                  >
                    {child.icon && <child.icon size={14} className="mr-2 shrink-0" />}
                    <span className="truncate">{child.translationKey ? t(child.translationKey, { defaultValue: child.label }) : child.label}</span>
                  </Link>
                );
              }
              return null;
