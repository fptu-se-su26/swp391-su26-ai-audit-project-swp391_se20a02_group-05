...
│   ├── TripGenie.API.Benchmarks/       # High-performance code path bench markers
│   ├── TripGenie.API.IntegrationTests/ # End-to-End API test executions
│   ├── TripGenie.API.PerformanceTests/ # Stress and concurrent load tests
│   └── TripGenie.API.UnitTests/        # Isolated mock service test suites
├── Program.cs              # Global Application Bootstrapper & Dependency Injection Container
├── appsettings.json        # Static base configurations
└── TripGenie.sln           # Multi-project solution configuration file
```
...
| **`DB_NAME`** | Yes | `tripgenie_db` | Name of the primary database. |
...
| **`EMAIL_SENDER_EMAIL`**| Yes| `dev@tripgenie.ai` | Sender address shown in system emails. |
...
*   **Add a new migration**:
    ```bash
    dotnet ef migrations add NameOfYourMigration --project TripGenie.API.csproj
    ```
*   **Manually apply migrations to database**:
    ```bash
    dotnet ef database update --project TripGenie.API.csproj
    ```
...
TripGenie uses a highly secure, stateless token authentication architecture utilizing dual HttpOnly cookies and IP-partitioned rate limiting filters.
...
To guarantee email delivery even during network failures, TripGenie separates database transactions from actual network tasks:
